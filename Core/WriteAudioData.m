function success = WriteAudioData(signal,sample_rate,file_path)
%
...
if strfind(file_path,'.kanwal')
    if sample_rate ~= 2.5e5
        %resample cannot handle the large sample numbers so these are
        ...
        switch(sample_rate)
            ...
            otherwise
                errordlg('Sampling rate non-standard, try writing wav file')
                success = 0;
                return
        end
    end
    ...
    written = fwrite(fid,signal,'int16');
    fclose(fid);
